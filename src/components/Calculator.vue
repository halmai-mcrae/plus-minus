<template>
  <Grid
    ...
    cols="4"
    gap="2"
    class="p-2 rounded-lg bg-gray-700 w-full my-10 sm:max-w-xl"
  >
    <Screen :text="memory" />
...
export default defineComponent({
  ...
  setup: () => {
    ...
    function calculateResult() {
      ...
      const hasValidOperation = mathExpression
        .split("")
        .findIndex((char) => isOperator(char));
    ...
    const memoryNormalized = computed(() => memory.value.replace("s", "-"));
    return {
